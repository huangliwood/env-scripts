...
class PerfCounters(object):
    ...
    def file_init(self, filename: str, init_spec: bool = False):
        ...
        self.flag_id = f"{time.strftime('%m-%d-%H:%M',time.localtime())}"
        if init_spec:
...
def merge_perf_counters(all_manip,all_perf, verbose=False):
    ...
    if is_deal_one:
        yield ["trace","flag_id"] + list(all_perf[0].dump_counters.keys())
        yield [all_perf[0].spec_name,all_perf[0].flag_id] + list(all_perf[0].dump_counters.values())
    else:
...
if __name__ == "__main__":
    ...
    def perf_work(manip, work_queue, perf_queue):
        while not work_queue.empty():
            try:
                item = work_queue.get()
                perf = PerfCounters(item,False if is_deal_one else True)
